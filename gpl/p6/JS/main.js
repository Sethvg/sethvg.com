...
app.controller("main", ['$rootScope', '$scope', '$location', function ($rootScope, $scope, $location) {
    ...
    $scope.toggle = function (t) {
        if (t == 'directionOn') {
            ...
            lightOn = !lightOn;
            lightOBJ.inverseNormals();

        }
        $scope.lightOn = lightOn;
        $scope.directionOn = directionOn;
    };
    ...
    //     Used to set up drawables and create objects.
    //*****************************************//
    $scope.init = function () {
        //Create the contexts
        ...
        var cone2 = new Drawable();
        cone2.makeCone(-50, 50, 0, 20, 40);
        cone2.setColor(1, 1, 1);
        drawables.push(cone2);
        //cone2.printNormals();
        ...
        for (var a = 0; a < 360; a++) {
            ...
            mountains.addFace(new Float32Array([p1[0], p1[1], -2, mp[0], mp[1], mHeight, p2[0], p2[1], -2]), new Vector([-mp[0], -mp[1], 0]));
            mountains.shader = 'none';
        ...
        for (var a = 0; a <= 360; a++) {
            ...
            if (a != 0) sun.addFace(new Float32Array([old[0], sunnyD, old[1], 0, sunnyD, 0, mp[0], sunnyD, mp[1]]), new Vector([1, 1, 1]));
        ...
        lightOBJ = new Drawable();
        lightOBJ.setColor(1, 1, 0);
        lightOBJ.makeCone(-4, 0, 2, .125, .25, 360);
        lightOBJ.makeCone(-4, 0, 2, .125, -.25, 360);
        lightOBJ.inverseNormals();
        drawables.push(lightOBJ);
